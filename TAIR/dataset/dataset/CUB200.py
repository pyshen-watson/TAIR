...
class CUB200(Dataset):
    ...
    root: Path  # The root directory of the dataset. Ex. 'data/CUB_200_2011'
    ...
    def __post_init__(self):
        ...
        if not self.clear_dir.exists():
            logging.error(f"The clear image directory: {self.clear_dir} does not exist.")
            exit()

        if not self.degraded_dir.exists():
            logging.error(f'The {self.dg_type} image directory "{self.degraded_dir}" does not exist.')
            logging.error("You should run the create_degraded_images.py first.")
            exit()
